...
#pattern = "['Sector Pattern Single-Unit']"
pattern = "['Square Pattern Single-Unit']"
pattern = "['Parallel Single-Unit Spiral']"
...
if pattern == "['Square Pattern Single-Unit']":
    ...
    #plt.xlim([-0.25,1])
    #plt.ylim([-0.25,1])
    plt.title("Square Search Pattern")
    plt.xlabel("x (m)")
    plt.ylabel("y (m)")
...
if pattern == "['Sector Pattern Single-Unit']":
    ...
    x_sector = np.array([h*np.cos(4*np.pi/3),h*np.cos(np.pi/3),h*np.cos(0),h*np.cos(np.pi),h*np.cos(2*np.pi/3),h*np.cos(5*np.pi/3),h*np.cos(4*np.pi/3)])
    x_sector = x_sector+x_center
    y_sector = np.array([k*np.sin(4*np.pi/3),k*np.sin(np.pi/3),k*np.sin(0),k*np.sin(np.pi),k*np.sin(2*np.pi/3),k*np.sin(5*np.pi/3),k*np.sin(4*np.pi/3)])
    y_sector = y_sector+y_center
    
    
    plt.plot(x_sector,y_sector,'g-')
    plt.plot(x_center + h*np.cos(t), y_center + k*np.sin(t), 'r--')
    plt.title("Sector Pattern")
    plt.xlabel("x (m)")
    plt.ylabel("y (m)")    
    #plt.xlim([0,0.5])
    #plt.ylim([0,0.5])
...
if pattern == "['Parallel Single-Unit Spiral']":
    ...
    plt.plot(x_spiral,y_spiral,'g-')
    #plt.plot(testx,testy,'b-')
    plt.title("Parallel Spiral Pattern")
    plt.xlabel("x (m)")
    plt.ylabel("y (m)")     
    #plt.xlim([0,0.75])
    #plt.ylim([0,0.75])
